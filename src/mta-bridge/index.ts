...
//
// Run as its own docker-compose service (`mta-bridge`) using this same image - see docker-compose.mail.yml.
...
// `relay_domains = tcp:mta-bridge:<domainPort>` - consulted once per RCPT TO, before relay_recipient_maps.
...
// `relay_recipient_maps = tcp:mta-bridge:<recipientPort>` - the actual mailbox/distribution-list existence
...
// `transport_maps = static:smtp:mta-bridge:<smtpPort>` - every accepted message's actual delivery hop.
...
async function start(): Promise<void> {
    ...
    console.log(
        `mta-bridge listening: domain=${domainPort} recipient=${recipientPort} smtp=${smtpPort}, ` +
            `forwarding to ${ingestBaseUrl}`,
    );
}

void start().catch((err) => {
    console.error("mta-bridge failed to start:", err);
    process.exit(1);
});

const shutdown = async (): Promise<void> => {
    console.log("mta-bridge shutting down...");
