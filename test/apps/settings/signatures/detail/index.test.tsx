...
import SignatureDetailPage from "../../../../../apps/www/settings/signatures/detail/index.js";
...
beforeEach(() => {
    window.history.pushState(null, "", "/settings/signatures/detail?uid=sig1&mailboxUid=mb1");
});
...
describe("SignatureDetailPage", () => {
    it("shows an alert when no uid is given", async () => {
        window.history.pushState(null, "", "/settings/signatures/detail?mailboxUid=mb1");
        mockShell();
        render(<SignatureDetailPage userUid="u1" />);
        expect(await screen.findByText("No signature specified.")).toBeInTheDocument();
    });
