...
import BookingTypeDetailPage from "../../../../../apps/www/settings/booking-types/detail/index.js";
...
beforeEach(() => {
    window.history.pushState(null, "", "/settings/booking-types/detail?uid=bt1&mailboxUid=mb1");
});
...
describe("BookingTypeDetailPage", () => {
    it("shows an alert when no uid is given", async () => {
        window.history.pushState(null, "", "/settings/booking-types/detail?mailboxUid=mb1");
        mockShell();
        render(<BookingTypeDetailPage userUid="u1" />);
    ...
    it("loads and pre-fills the form from the existing booking type", async () => {
        ...
        expect(screen.getByText(/\/book\?slug=intro-call/)).toBeInTheDocument();
    });
    ...
    // resolution above depend on the real one — `mockLocation`'s replacement isn't undone between tests
    // (unlike `vi.stubGlobal`). Must run last in this file, same convention as
    // `apps/admin/domains/detail/index.test.tsx`'s own delete-navigation test.
