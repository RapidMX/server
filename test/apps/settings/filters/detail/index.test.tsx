...
import MailFilterDetailPage from "../../../../../apps/www/settings/filters/detail/index.js";
...
beforeEach(() => {
    window.history.pushState(null, "", "/settings/filters/detail?uid=mfr1&mailboxUid=mb1");
});
...
describe("MailFilterDetailPage", () => {
    it("shows an alert when no uid is given", async () => {
        window.history.pushState(null, "", "/settings/filters/detail?mailboxUid=mb1");
        mockShell();
        render(<MailFilterDetailPage userUid="u1" />);
        expect(await screen.findByText("No mail filter specified.")).toBeInTheDocument();
    });
