...
import MailboxDetailPage from "../../../../../apps/admin/mailboxes/detail/index.js";
...
beforeEach(() => {
    window.history.pushState(null, "", "/admin/mailboxes/detail?uid=mb1");
});
...
describe("MailboxDetailPage", () => {
    it("shows an alert when no uid is given", async () => {
        window.history.pushState(null, "", "/admin/mailboxes/detail");
    ...
    it("shows an error message when impersonation fails", async () => {
        ...
        render(<MailboxDetailPage userUid="admin-1" impersonationBaseUrl="https://auth.example.com" />);
    ...
    // Mocks window.location wholesale (see testUtils.mockLocation) — must run last in this file, since
    // every other test here relies on a real, pushState-driven window.location to read `?uid=` on mount.
    it("impersonates via this app's own local endpoint and redirects when impersonationBaseUrl isn't provided", async () => {
        ...
        const user = userEvent.setup();
        render(<MailboxDetailPage userUid="admin-1" />);

        // Mocked only after mount: readTargetUid() needs the real, pushState-driven window.location to
        // resolve `?uid=` on the initial render — see testUtils.mockLocation's doc comment.
