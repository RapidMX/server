 ...
 * (`${mail:booking:public_url}/manage/:token`) — this framework's file-based `ReactRoute` routing has no
 * dynamic route segments at all (confirmed by reading `resolveAppFile()`: it requires an exact file match
 * for the request path, no catch-all/parameterized fallback), so that link can never resolve here. This
 * page therefore reads the token from the query string instead (`/book/manage?token=...`), and
 * `mail:booking:public_url` is deliberately left unset so the confirmation email carries no broken link —
 * the *working* manage link is the one `apps/book/index.tsx` shows on-screen right after a booking,
 * built from `bookSlot()`'s own response (see `bookingManageUrl()` in `bookingApi.ts`).
 */
