...
/** This framework has no dynamic route segments — the target booking type's uid comes from the query
 * string instead, same convention as every other detail page (e.g. `settings/filters/detail`). */
export function readTargetUid(): string | null {
    if (typeof window === "undefined") return null;
    return new URLSearchParams(window.location.search).get("uid");
}

export type BookingTypeDetailPageProps = Omit<SettingsShellProps, "active">;

export default function BookingTypeDetailPage(props: BookingTypeDetailPageProps) {
    return (
        <SettingsShell {...props} active="booking-types">
            <BookingTypeDetailContent />
        </SettingsShell>
    );
}

function BookingTypeDetailContent() {
    ...
    if (!uid) {
        return <Alert>No booking link specified.</Alert>;
    }
    if (loading) {
    ...
    if (error || !bookingType) {
        ...
        // `null`) is therefore unreachable, unlike e.g. `messages/detail/index.tsx`'s identical-looking
        // check, which stores its fetched value whole rather than destructuring it.
        return (
            <div className="p-6">
                <Alert>{error!}</Alert>
            </div>
    ...
    const publicUrl = `${window.location.origin}/book?slug=${encodeURIComponent(bookingType.slug)}`;
