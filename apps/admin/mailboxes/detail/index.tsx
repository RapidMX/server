 ...
 * mailbox's uid comes from the query string instead, same convention as auth-server's admin user detail page.
 */
...
function MailboxDetailContent({ impersonationBaseUrl }: Pick<AdminShellProps, "impersonationBaseUrl">) {
    ...
    if (!uid) {
        return <Alert>No mailbox specified.</Alert>;
    }
    if (loading) {
