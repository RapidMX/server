...
import { dayDropId, eventDragId, resizeDragId, slotDropId } from "../../lib/calendarDragIds.js";
...
/** An hourly time grid (Week or Day view, sharing all their rendering/drag logic — they differ only
 * in how many day columns are shown), with all-day events in a header strip above it. */
export default function TimeGridView({ days, occurrences, onSelectEvent, onSelectSlot }: TimeGridViewProps) {
                            ...
                            {allDayEvents
                                ...
                                .map((occ) => (
                                    <button
                                        ...
                                        className="text-xs text-left truncate rounded-sm px-1.5 py-0.5 bg-primary/15 text-primary-dark"
                                    >
                                        {occ.title}
                                    </button>
                                ))}
...
function DayColumn({ day, occurrences, onSelectEvent, onSelectSlot }: DayColumnProps) {
    ...
    return (
        <div className={["flex-1 min-w-0 relative border-l border-border", isToday(day) ? "bg-primary/5" : ""].join(" ")}>
            ...
            {occurrences.map((occurrence) => (
                <EventBlock key={occurrence.occurrenceKey} occurrence={occurrence} dayStart={dayStart} onSelect={onSelectEvent} />
            ))}
        </div>
    );
}
...
function EventBlock({ occurrence, dayStart, onSelect }: { occurrence: CalendarOccurrence; dayStart: Date; onSelect: (occurrence: CalendarOccurrence) => void }) {
